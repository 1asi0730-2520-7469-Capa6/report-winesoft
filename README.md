...
| **US02** | Diseño Responsivo | Como visitante, quiero que la landing page se vea bien en any dispositivo (móvil, tablet, escritorio), para tener una buena experiencia de usuario. | **CA1:** Dado que se accede desde un móvil, cuando la página carga, entonces el contenido se reorganiza en una sola columna.<br>**CA2:** Dado que se cambia el tamaño de la ventana del navegador, cuando se ajustan las dimensiones, entonces los elementos se reajustan fluidamente. | EP01 |
...
| **TS07** | Sistema de notificaciones | Implementar componente visual de notificaciones, mostrando alertas en tiempo real sobre stock o vencimiento. | **CA1:** Dado que se genera una alerta, cuando el usuario abre sesión, entonces ve el ícono de notificación activa.<br>**CA2:** Dado que revisa la alerta, cuando hace clic, entonces se marca como “leída”. | EP03 |
...
| 2 | **US17** | Explorar información de WineSoft | Como visitante, quiero conocer los servicios y beneficios de WineSoft para decidir si registrarme. | **3** |
...
| 4 | **US18** | Enviar formulario de contacto | Como visitante, quiero enviar mis datos para que el equipo de WineSoft me contacte. | **3** |
| 5 | **US02** | Diseño Responsivo | Como visitante, quiero que la landing page se vea bien en cualquier dispositivo (móvil, tablet, escritorio), para tener una buena experiencia. | **3** |
| 6 | **US04** | Registrar nuevo insumo | Como usuario, quiero añadir nuevos insumos con nombre, cantidad, unidad y fecha de vencimiento para mantener mi inventario actualizado. | **8** |
| 7 | **US05** | Editar stock existente | Como usuario, quiero actualizar la cantidad o detalles de un insumo para reflejar los cambios en el inventario real. | **3** |
| 8 | **US07** | Buscar y filtrar insumos | Como usuario, quiero buscar y filtrar mis insumos por nombre, tipo o fecha, para encontrarlos rápidamente. | **3** |
| 9 | **US06** | Eliminar insumo | Como usuario, quiero eliminar un insumo que ya no utilizo, para mantener limpio y actualizado mi inventario. | **2** |
| 10 | **US08** | Crear pedido de envío | Como proveedor, quiero generar pedidos según las alertas de mis clientes para organizar el abastecimiento. | **8** |
| 11 | **US09** | Actualizar estado del transporte | Como proveedor, quiero actualizar el estado del envío para mantener la trazabilidad. | **3** |
| 12 | **US10** | Ver historial de entregas | Como proveedor, quiero consultar el historial de pedidos enviados y entregados. | **2** |
| 13 | **US13** | Configurar umbrales de alerta | Como usuario, quiero definir los valores mínimos y días de vencimiento para recibir notificaciones personalizadas. | **8** |
| 14 | **US12** | Alertas de stock bajo | Como usuario, quiero recibir notificaciones cuando un insumo esté bajo el umbral mínimo. | **3** |
| 15 | **US11** | Alertas de vencimiento | Como usuario, quiero recibir alertas cuando un producto esté próximo a vencer. | **3** |
| 16 | **US14** | Visualizar gráficos de stock | Como usuario, quiero ver gráficos actualizados sobre mi inventario para identificar tendencias. | **3** |
| 17 | **US16** | Comparar desempeño histórico | Como usuario, quiero comparar mi desempeño mensual o anual para detectar mejoras o caídas. | **5** |
| 18 | **US15** | Exportar reportes | Como usuario, quiero exportar mis reportes en PDF o Excel para análisis externo. | **3** |
| 19 | **US19** | Obtener lista de inventario (GET /inventory) | Como desarrollador, quiero acceder a la lista de insumos registrados mediante la API. | **3** |
| 20 | **US20** | Crear pedido vía API (POST /orders) | Como desarrollador, quiero registrar nuevos pedidos de envío mediante la API. | **3** |
| 21 | **US21** | Consultar trazabilidad (GET /tracking) | Como desarrollador, quiero consultar el estado y ubicación de un pedido mediante la API. | **3** |
