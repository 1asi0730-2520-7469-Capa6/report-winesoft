...
| 17 | **US17** | Obtener lista de inventario (GET /inventory) | Como desarrollador, quiero acceder a la lista de insumos registrados mediante la API. | **3** |
| 18 | **US18** | Crear pedido vía API (POST /orders) | Como desarrollador, quiero registrar nuevos pedidos de envío mediante la API. | **3** |
| 19 | **US19** | Consultar trazabilidad (GET /tracking) | Como desarrollador, quiero consultar el estado y ubicación de un pedido mediante la API. | **3** |
| 20 | **US21** | Validación de Token en API | Como desarrollador, quiero que la API valide el token en cada solicitud protegida, para asegurar el acceso solo a usuarios autenticados. | **3** |
| 21 | **US20** | Mantener sesión activa (Token Refresh) | Como usuario, quiero que mi sesión permanezca activa mientras estoy usando la app, para no tener que iniciar sesión repetidamente. | **5** |
| 22 | **US22** | Cerrar Sesión | Como usuario, quiero poder cerrar mi sesión activamente, para asegurar que nadie más pueda acceder a mi cuenta. | **2** |
